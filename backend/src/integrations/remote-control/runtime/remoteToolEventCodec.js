...
function extractCommandFromArgs(argsObject) {
  ...
  return typeof argsObject.command === "string" ? argsObject.command.trim() : "";
}
...
function normalizeHookLevel(value) {
  ...
  if (text === "hint" || text === "info" || text === "warning") {
    return text;
  }
  return "hint";
}
...
function normalizeHooks(hooks) {
  ...
  return hooks
    .map((hook, index) => {
      ...
      return {
        id:
          (typeof hook.id === "string" && hook.id.trim()) || `remote_hook_${index + 1}`,
        type:
          (typeof hook.type === "string" && hook.type.trim()) || "generic_hint",
...
export function createRemoteToolMessagePayloadFromCall(event) {
...
export function createRemoteToolMessagePayloadFromResult(event) {
  return {
    format: "tool_event",
    version: 2,
    toolCallId: normalizeToolCallId(event?.toolCallId),
...
export function applyRemoteToolPendingApprovalToPayload(payload, event) {
  return {
    ...payload,
...
export function applyRemoteToolResultToPayload(payload, event) {
...
export function parseRemoteToolMessagePayload(content) {
...
export function serializeRemoteToolMessagePayload(payload) {
