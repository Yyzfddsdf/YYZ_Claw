...
export function createLongTermMemorySystemPrompt() {
  return [
    ...
    "memory_find_candidates 用于在写入前查找最相近的现有 memory node 候选，并决定优先 update、merge 还是 create 新 node。",
    ...
    "如果 memory_find_candidates 已给出高相似 nodeCandidates，优先 update 或 merge；父级挂载位置单独用 memory_browse 确认。",
